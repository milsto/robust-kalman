...
xres_history = list()
z_history = list()
x_kalman_history = list()
inov_history = list()
cee_x_history = list()
cee_xres_history = list()
cee_x = 0.0
cee_xres = 0.0
step = 2
...
for t in t_axis:
    x_history.append(x)
    z_history.append(z)
    x_kalman_history.append(kalman_linear.x)
    xres_history.append(kalman_robust.x)
    ...
    cee_x_history.append(cee_x)
    cee_xres_history.append(cee_xres)
    ...
    inov_history.append(kalman_robust.inovation)

    step += 1

plt.figure()
plt.plot(t_axis, [x[0, 0] for x in x_history], 'g', label='x0')

plt.hold(True)

plt.plot(t_axis, [z[0, 0] for z in z_history], 'b', label='z0')

plt.plot(t_axis, [k[0, 0] for k in x_kalman_history], 'r', label='x est')

plt.plot(t_axis, [k[0, 0] for k in xres_history], 'k', label='x est robust')

plt.legend()

plt.figure()
plt.plot(t_axis, cee_x_history / np.arange(1, len(cee_x_history) + 1, 1), 'r')
plt.hold(True)
plt.plot(t_axis, cee_xres_history / np.arange(1, len(cee_xres_history) + 1), 'k')

plt.figure()
plt.plot(t_axis, [k[0, 0] for k in inov_history], 'k')
plt.title('inovation')
